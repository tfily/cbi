...
// Centralized fetch helper that fails gracefully in production builds
async function wpFetch(endpoint, options = {}) {
  if (!WP_API_URL) {
    ...
    // return empty array by default so callers that expect arrays don't explode
    return [];
  }
  ...
  if (!res.ok) {
    ...
    // Instead of throwing, return [] to avoid breaking build
    return [];
  }
...
export async function getServiceBySlug(slug) {
  ...
  const items = await wpFetch(
    `/services?slug=${encodeURIComponent(slug)}&per_page=1`
  );
