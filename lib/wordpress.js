...
export async function getServiceBySlug(slug) {
  const services = await wpFetch(`/services?slug=${slug}&_embed`);
...
export async function getSubscriptionBySlug(slug) {
  const items = await wpFetch(`/subscriptions?slug=${slug}`);
  return Array.isArray(items) ? items[0] || null : null;
}

export async function getSubscriptionSlugs() {
  const items = await wpFetch("/subscriptions?per_page=100&_fields=slug");
  if (!Array.isArray(items)) return [];
  return items.map((item) => item.slug).filter(Boolean);
}
...
export async function getPostBySlug(slug) {
  if (!slug) return null;
  const items = await wpFetch(`/posts?slug=${encodeURIComponent(slug)}`);
  return Array.isArray(items) ? items[0] || null : null;
}

export async function getPostSlugs() {
  const items = await wpFetch("/posts?per_page=100&_fields=slug");
  if (!Array.isArray(items)) return [];
  return items.map((item) => item.slug).filter(Boolean);
}
...
export async function getPageBySlug(slug) {
  if (!slug) return null;
  const pages = await wpFetch(`/pages?slug=${encodeURIComponent(slug)}&_embed`);
  return Array.isArray(pages) ? pages[0] || null : null;
}

export async function getAboutPage() {
  const slugs = ["about-us", "a-propos", "about"];
